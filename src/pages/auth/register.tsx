...
import { registrationSchema, registrationType } from '@propstypes/schemas/register.schema';
            ...
            <form method='post' onSubmit={handleSubmit(formSubmit)}>
              <Stack direction="column">
                ...
                <Stack direction="row" gap={4}>
                    ...
                    <FormControl my={2} isRequired isInvalid={'firstName' in errors}>
                      <FormLabel fontSize="sm" color="gray.700">Nom</FormLabel>
                      <Input type='text' {...register('firstName')} />
                      <FormErrorMessage>{errors.firstName?.message}</FormErrorMessage>
                    </FormControl>
                    <FormControl my={2} isRequired isInvalid={'lastName' in errors}>
                      <FormLabel fontSize="sm" color="gray.700">Prénom</FormLabel>
                      <Input type='text' {...register('lastName')} />
                      <FormErrorMessage>{errors.lastName?.message}</FormErrorMessage>
                    </FormControl>
                </Stack>
                <FormControl my={2} isRequired isInvalid={'email' in errors}>
                  <FormLabel fontSize="sm" color="gray.700">Adresse e-mail</FormLabel>
                  <Input type="email" {...register('email')} />
                  <FormErrorMessage>{errors.email?.message}</FormErrorMessage>
                </FormControl>
                <Stack direction="row" gap={4}>
                  <FormControl flex={2} my={2} isRequired isInvalid={'username' in errors}>
                    <FormLabel fontSize="sm" color="gray.700">Utilisateur</FormLabel>
                    <Input type='text' {...register('username')} />
                    <FormErrorMessage>{errors.username?.message}</FormErrorMessage>
                  </FormControl>
                  <FormControl isInvalid={'gender' in errors} flex={1} my={2} isRequired>
                    <FormLabel fontSize="sm" color="gray.700">Sexe</FormLabel>
                    <RadioGroup onChange={val => setValue('gender', val)}>
                      <Stack direction="column" gap={0}>
                        <Radio value="man"><Text fontSize="sm" color="gray.700">Homme</Text></Radio>
                        <Radio value="woman"><Text fontSize="sm" color="gray.700">Femme</Text></Radio>
                      </Stack>
                    </RadioGroup>
                  </FormControl>
                </Stack>
