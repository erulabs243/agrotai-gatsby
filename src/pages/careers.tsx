import { Heading, Stack, Box, Text, Image, SimpleGrid, Link, Button, Icon } from '@chakra-ui/react'
    ...
    <Layout>
      ...
      >
        <Stack as={Box} spacing={4} w={{ base: "full", md: "40%" }}>
          <Fade delay={1000}>
          <Slide cascade >
          <Heading as="h2" mb={2} fontWeight={700} textAlign={{base: 'center', lg: 'left'}}>
            Travailler{" "}
        ...
        {careers.nodes.length <= 0 && <Text>Aucune nouvel article</Text>}
            {careers.nodes.length > 0 && <SimpleGrid columns={{base: 1, md: 2}} 
                        columnGap={{base: 4, lg: 8}}
                        maxW={{base: "90vw", lg: '3xl'}}
                        mx="auto"
                        py={{base: 4, lg: 12}}>
                {
                    careers.nodes.map(career => <Stack 
                        ...
                        <Stack direction="column" gap={0} flex={1}>
                            ...
                            <Link
                                ...
                                noOfLines={2}
                                color="gray.700"
                            >{career.description}</Link>
                        </Stack>
                        <Stack direction="row" alignItems="center" justifyContent="space-between">
                            <Text fontSize="sm" color="gray.600">{formatDate(career.updatedAt)}</Text>
                            <Button 
